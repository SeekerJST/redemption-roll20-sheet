...
- **`RedemptionBleedRound.js`** — a fourth **Mod (API) script** (GM only). `!applybleeds` runs the start-of-round bleed tick for every combatant at once: it bumps a hidden `bleedTick` attribute on each target, and the sheet worker's `change:bleedTick` handlers run the existing Character/Starship/Unit apply (no server-side duplication). Targets = selected tokens, else the turn tracker; only ticks sheets with **Use Automation** on. Requires Pro + the sheet.
- **`RedemptionScatter.js`** — a fifth **Mod (API) script** (GM only), a hazard tool. `!scatter` flings every affected token (selected, else the players'-page objects layer): 2d8 → direction (1 N … 8 NW) + distance, bounce (+3 and reflect) if it would leave the map, and wipes the token's `DefMargin`. Requires Pro.
